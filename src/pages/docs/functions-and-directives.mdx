...
使用 `@apply` 把你自定義的 CSS 列入已存在的功能 class。
...
如果你希望使用 `@apply` 在已經存在的 class 並且給它 `!important`，只要在宣告的結尾加上 `!important` 即可：
...
將自定義的 CSS 使用 `@layer` 囊括著也可以告知 Tailwind 在清除不必要的 CSS 會來檢查這些樣式，詳細可以閱讀我們的 [生產環境優化](/docs/optimizing-for-production)。
